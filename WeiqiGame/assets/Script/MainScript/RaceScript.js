...
cc.Class({
     ...
     */
    onLoad: function () {
        cc.log('scoreeeeeeee:' + this.score);
        cc.log('Common Mode ChessOnLoad');
        this._initMapPos();
        var self = this;
        ...
        for (var y = 0; y < this.boardHeight; y++) {
            for (var x = 0; x < this.boardWidth; x++) {
                ...
                //cc.log('根据棋盘和棋子大小计算使每个棋子节点位于指定位置');
                newNode.setPosition(cc.v2(x * 50 + 50, y * 50 + 50));
    ...
    start() {
        ...
        this._leftScore = this.leftPanal.getChildByName('Score');
    },

    _initMapPos: function () {
        console.log('mission', gameManager.mission);
    ...
    update(dt) {
        this._leftScore.getComponent(cc.Label).string = this.score;
    },


    _judgeOverNew: function () {
        cc.log('Calling method [_judgeOverNew]');
        ...
        this._fireSpread(this.blackAnthor);
        this._setBlackChessList(this.changeFireArray);
        ...
        cc.log('newBreath: ' + newBreath);
        if (breath == 1 && newBreath != 0) {
            this.blackAnthor = this._circleCacular();
            this._breathCacular(this.blackAnthor);
        }
        ...
        if (breath == 0) {
            ...
            cc.log('count:', count, 'nextblack:', this.nextBlackIndex);
    ...
    _breathCacular: function (blackAnthor) {
        console.log('_breathCacular');
        let current = blackAnthor;
        var breath = 0;
        console.log('锚点:', current);
        // 统计锚点火焰气的个数
        // 左边界
        console.log('L');
        ...
        console.log('breath：', breath, 'this.nextBlackIndex:', this.nextBlackIndex);
        return breath;
    },
     ...
     */
    initGame: function () {
        // this.chessList[101].getComponent(cc.Sprite).spriteFrame = this.bigFire;
        // this.setFiremanSpriteFrame(this.chessList[1]);
        // this.setFiremanSpriteFrame(this.chessList[2]);
        // this.setFiremanSpriteFrame(this.chessList[100]);
        // this.blackAnthor = 101;
    ...
    _judgeBoundaryNew: function () {
        cc.log('Calling method [_judgeBoundaryNew]');
        //当前棋子坐标
        let current = this.touchChess.getPosition();
        cc.log('current:', current);
        //游戏区域宽度
        let parentWidth = this.parentNode.width;
        cc.log('parentWidth:', parentWidth);
        //游戏区域高度
        let parentHeight = this.parentNode.height;
        cc.log('parentHeight:', parentHeight);
        //地图格子宽度
        let tileSize = this._tiledMap.getTileSize();
        cc.log('horizontalOffset:', horizontalOffset);
        cc.log('verticalOffset:', verticalOffset);
        ...
        if (current.y - verticalOffset < this.mapMoveStep * tileSize.width) {
            cc.log('up--', 'current.y:', current.y, 'verticalOffset:', verticalOffset); //-110?
        ...
        cc.log('Return method [_judgeBoundaryNew]');
    },
    ...
    _tryMoveMapNew: function (moveDir) {
        cc.log('Calling method [_tryMoveMapNew]');
        //地图坐标
        ...
        cc.log('player:', player, 'disX:', disX, 'disY:', disY);
        ...
        cc.log('Return method [_tryMoveMapNew]');
    },
    ...
    LeftButton: function () {
        console.log('左键inter');
        console.log('nextBlackIndex.length:', this.nextBlackIndex.length);
        var chooseChess;
        if (this.nextBlackIndex.length === 2) {
            var maxChessIndex = this._getArrMax();
            console.log('下一步最大索引' + maxChessIndex);
            var minChessIndex = this._getArrMin();
            console.log('下一步最小索引' + minChessIndex);
            if (maxChessIndex - minChessIndex > 0) {
                if ((maxChessIndex - minChessIndex) % this.boardWidth == 0) {
                    chooseChess = this.chessList[maxChessIndex];
                }
                else if ((maxChessIndex - minChessIndex) / this.boardWidth > 1) {
                    //不停-this.size 直到|max - min| < this.size
                    chooseChess = this.chessList[this._getAbs(minChessIndex, maxChessIndex, "L")];
                }
                else if ((maxChessIndex - minChessIndex) / this.boardWidth < 1) {
                    if (this._ergodic(minChessIndex, maxChessIndex)) {
                        console.log("1");
                        chooseChess = this.chessList[maxChessIndex];
                    }
                    else {
                        console.log("2");
                        chooseChess = this.chessList[minChessIndex];
                    }
                }
            }
        }
        if (this.nextBlackIndex.length === 1) {
            chooseChess = this.chessList[this.nextBlackIndex[0]];
        }
        if (this.nextBlackIndex.length >= 3) {
            this._gameOver();
            return;
        }

        this.touchChess = chooseChess;
        console.log('this.touchChess:', this.touchChess);
        if (this.gameState === 'white' && this.touchChess.getComponent(cc.Sprite).spriteFrame === null) {
            //下子后添加棋子图片使棋子显示
            console.log('落白子');
            this.setFiremanSpriteFrame(this.touchChess);
            this.score = this.score + 1;
        }
        this.whiteChessList.push(this.touchChess);
        console.log('whiteChessList:', this.whiteChessList);

        this._judgeOverNew();
        this.computer();
        this._resetFiremanFrame();
        cc.log('本次积分：' + this.score);
    },
    ...
    RightButton: function () {
        ...
        if (this.nextBlackIndex.length === 2) {
            ...
            if (maxChessIndex - minChessIndex > 0) {
                if ((maxChessIndex - minChessIndex) % this.boardWidth == 0) {
                    ...
                    //不停-this.size 直到|max - min| < this.size
                    console.log('RTN');
                    chooseChess = this.chessList[this._getAbs(minChessIndex, maxChessIndex, "R")];
                    console.log('chooseChess:' + this._getAbs(minChessIndex, maxChessIndex, "R"));
                }
                else if ((maxChessIndex - minChessIndex) / this.boardWidth < 1) {
                    if (this._ergodic(minChessIndex, maxChessIndex)) {
                        chooseChess = this.chessList[minChessIndex];
        ...
        console.log('this.touchChess:', this.touchChess);
        if (this.gameState === 'white' && this.touchChess.getComponent(cc.Sprite).spriteFrame === null) {
            //this.touchChess.getComponent(cc.Sprite).spriteFrame = this.whiteSpriteFrame;
            this.setFiremanSpriteFrame(this.touchChess);
            this.score = this.score + 1;
        }
        this.whiteChessList.push(this.touchChess);
    ...
    _setBlackChessList: function (changeFireArray) {
        cc.log('calling _setBlackChessList, param length:' + changeFireArray.length);
    ...
    _circleValid: function () {
        cc.log('Calling method [_circleValid]');
        let count = 0;
        for (let i = 0; i < this.changeFireArray.length; i++) {
            cc.log('current node:' + this.changeFireArray[i]);
            let breath = this._breathCacular(this.changeFireArray[i]);
            count = breath + count;
            cc.log('count:' + count);
        }
    ...
    _gameOver: function () {
        ...
        this._endScore.getComponent(cc.Label).string = this.score;
        //按钮控制
    ...
    _getArrMax: function () {
        return Math.max.apply(Math, this.nextBlackIndex);
    },

    _getArrMin: function () {
        return Math.min.apply(Math, this.nextBlackIndex);
    },
    ...
    /**
     * 遍历两索引间所有边界数字
     * @param {当前轮次最小棋子索引} minIndex 
     * @param {当前轮次最大棋子索引} maxIndex 
     */
    _ergodic: function (minIndex, maxIndex) {
        cc.log("_ergodic");
        let arr = [];

        let tmp = this.boardWidth - 1;
        while (true) {
            arr.push(tmp);
            tmp = tmp + this.boardWidth;
            if (tmp > this.boardHeight * this.boardWidth - 1) {
                break;
            }
        }
        //var arr = [18,37,56, 75, 94, 113, 132, 151, 170, 189 ,208, 227, 246, 265, 284, 303, 322, 341, 360];
     ...
     * @param {当前轮次最小棋子索引} minIndex 
     * @param {当前轮次最大棋子索引} maxIndex 
     * @param {当前点击键盘方向 L:左  R:右} direct 
     */
    _getAbs: function (minIndex, maxIndex, direct) {
        let flag = true;
        let temp = maxIndex;
        while (flag) {
            temp = temp - this.boardWidth;
            if (Math.abs(temp - minIndex) < this.boardWidth) {
                flag = false;
                if (this._ergodic(minIndex, temp) && direct == "L") {
                    return maxIndex;
                }
                else if (direct == "L") {
                    return minIndex;
                }
                else if (this._ergodic(minIndex, temp) && direct == "R") {
                    return minIndex;
                }
                else if (direct == "R") {
                    return maxIndex;
